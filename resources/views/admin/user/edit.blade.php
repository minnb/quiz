...
<div class="container-fluid card">
	<br>
	<form name="create-question" action="{{ route('post.admin.user.edit', ['id'=>fencrypt($id)])}}" method="POST" enctype="multipart/form-data">
		<input type="hidden" name="_token" value="<?php echo csrf_token() ?>">
		<div class="row backgroud_white">
			<div class="col-lg-4 col-md-4 col-xs-12 col-sm-12">
				<div class="form-group">
					<label>Name</label>
					<input type="text" name="name" class="form-control" required="" value="{{ old('name', isset($data) ? $data['name'] : 1) }}" />
				</div>
				<div class="form-group">
					<label>Email</label>
					<input type="email" name="email" class="form-control" required="" value="{{ old('email', isset($data) ? $data['email'] : 1) }}" readonly="" />
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<select class="form-control" name="role">
						{{ getSelectForm(App\Models\Role::getRoleSelect(),old('role', isset($data) ? App\Models\Role_User::getRoleIdFromUserId($data['id']) : 1)) }}
					</select>
				</div>
			</div>
		</div>
		...
		<div class="row backgroud_white">
			<div class="col-md-12 col-lg-12">
				...
				<a class="btn btn-primary" href="{{ route('get.admin.subject.list') }}"><i class="fa fa-list bigfonts" aria-hidden="true"></i> Danh sách Users</a>
			</div>
		</div>
	</form>
	<br>
